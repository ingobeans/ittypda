...
int sd_format(void) {
  ...
  FRESULT res;
  res = f_mkfs(sd_path, 1, 0, work, sizeof(work));
  return 0;
  if (res != FR_OK) {
    print("Format failed: f_mkfs returned %d\r\n", res);
  }
  print("success!");
  return res;
}
...
int sd_mount(void) {
  ...
  if (stat != 0) {
    print("disk_initialize failed: 0x%02X\n", stat);
    print("FR_NOT_READY Try Hard Reset or Check Connection/Power\r\n");
    print(
        "Make sure \"MX_FATFS_Init\" is not being called in the main function\n"
        "You need to disable its call in CubeMX->Project Manager->Advance "
        "Settings->Uncheck Generate code for MX_FATFS_Init\r\n");
    return FR_NOT_READY;
  }

  print("Attempting mount at %s...\r\n", sd_path);
  res = f_mount(&fs, sd_path, 1);
  if (res == FR_OK) {
    print("SD card mounted successfully at %s\r\n", sd_path);
  ...
  //	{
  //		print("No filesystem found on SD card. Attempting
  // format...\r\nThis will create 32MB Partition (Most Probably)\r\n");
  //		print("If you need the full sized SD card, use the computer
  // to
  // format into FAT32\r\n"); 		sd_format();
  //
  //		print("Retrying mount after format...\r\n");
  //		res = f_mount(&fs, sd_path, 1);
  //		if (res == FR_OK) {
  //			print("SD card formatted and mounted
  // successfully.\r\n"); 			print("Card Type: %s\r\n",
  // sd_is_sdhc() ? "SDHC/SDXC"
  //: "SDSC");
  //
  ...
  //		}
  //		else {
  //			print("Mount failed even after format: %d\r\n",
  // res);
  //		}
  //		return res;
  //	}
  ...
  print("Mount failed with code: %d\r\n", res);
  return res;
}

int sd_unmount(void) {
  FRESULT res = f_mount(NULL, sd_path, 1);
  print("SD card unmounted: %s\r\n", (res == FR_OK) ? "OK" : "Failed");
  return res;
}
...
int sd_read_csv(const char *filename, CsvRecord *records, int max_records,
  ...
  *record_count = 0;
  ...
  FRESULT res = f_open(&file, filename, FA_READ);
  if (res != FR_OK) {
    print("Failed to open CSV: %s (%d)", filename, res);
    return res;
  }
  ...
  for (int i = 0; i < *record_count; i++) {
    print("[%d] %s | %s | %d", i, records[i].field1, records[i].field2,
          records[i].value);
  }
